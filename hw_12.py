...
class AddressBook(UserDict):
    ...
    def search(self, search_str: str):
        """Возвращает список записей соответствующих критерию поиска"""
        lst = []
        for name, record in self.data.items():
            if search_str.title() in name:
                if record not in lst:
                    lst.append(record)
            for number in record.phone_list:
                if search_str in number.value:
                    if record not in lst:
                        lst.append(record)
        return lst
...
def unpacking_ab():
    """Распаковует сохранённый класс AddressBook из файла 'address_book.pic'
    Если файл существует, переопределяет переменную ADDRESS_BOOK"""
    global ADDRESS_BOOK
    try:
        with open('address_book.pic', 'rb') as f:
...
def packing_ab():
    ...
    with open('address_book.pic', 'wb') as f:
...
def search_command(command_lst: list):
    if len(command_lst) != 2:
        return 'Search string not specified'
    if len(command_lst[1]) < 3:
        return 'Please enter at least 3 characters'
    if search_list := ADDRESS_BOOK.search(command_lst[1]):
        text = 'Searching results:\n'
        for record in search_list:
            ls = []
            for phone in record.phone_list:
                ls.append(phone.value)
            text += f'{record.name.value} -> {", ".join(ls)}\n'
        return text
...
def main():
    ...
    while True:
        ...
        if result:
            ...
            print(f"Command not recognized. Use the command from the list:\n"
                  f"{', '.join(list(COMMAND_DICT.keys()))}")
